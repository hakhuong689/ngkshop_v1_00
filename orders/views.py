...
# Import dữ liệu từ app master
from products.models import Product, PrdUrl, PrdPrice
...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        """
        Override phương thức GET để xử lý dữ liệu mở rộng:
          - Dựa vào order_url của đơn hàng, lấy base URL.
          - Tìm PrdUrl có landing_url khớp với base URL.
          - Từ đó lấy product và dựa vào order_qty_amt (chính xác 100%) để tìm PrdPrice.
          - Nếu tìm thấy, thêm trường price_info vào kết quả trả về.
          - Nếu không, trường price_info sẽ là None.
        """
        ...
        for order in data:
            ...
            order_qty_amt = order.get('order_qty_amt', '')

            # Ban đầu, thêm trường price_info với giá trị None            
            order['product_code']   = None
            order['product_name']   = None
            order['price_number']   = None
            order['price_name']     = None
            order['price_amount']   = None

            if order_url and order_qty_amt:
                parsed_url = urlparse(order_url)
                # Xây dựng base URL: scheme://netloc + path (loại bỏ query string)
                base_url = f"{parsed_url.scheme}://{parsed_url.netloc}{parsed_url.path}" 

                try:
                    # Tìm bản ghi PrdUrl có landing_url khớp
                    prd_url = PrdUrl.objects.get(landing_url=base_url)
                    product = prd_url.product
                    order['product_code'] = f"{product.product_code}"
                    order['product_name'] = f"{product.product_name}"
                    
                    # So sánh chính xác order_qty_amt với landing_qty_amt của PrdPrice
                    prd_price = PrdPrice.objects.get(product=product, landing_qty_amt=order_qty_amt)
                    order['price_number'] = f"{prd_price.price_number}"                                        
                    order['price_name'] = f"{prd_price.price_name}"                    
                    order['price_amount'] = f"{prd_price.prd_amount}"

                except (PrdUrl.DoesNotExist, PrdPrice.DoesNotExist):
                    # Nếu không tìm thấy, giữ nguyên price_info là None
                    order['product_code']   = None
                    order['product_name']   = None
                    order['price_number']   = None
                    order['price_name']     = None
    ...
    # Các phương thức khác (create, partial_update, perform_create, v.v.) có thể giữ nguyên nếu không cần áp dụng logic tương tự
    # Do logic liên kết chỉ cần trả dịch qua GET chứ không cần cập nhật dữ liệu vào DB.
